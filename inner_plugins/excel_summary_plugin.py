...
class ExcelSummaryPlugin(ServicePlugin):
    def run(self, *args, **kwargs) -> Union[RunnerResult, T]:
        ...
        for save_path in save_path_list:
            if isinstance(save_path, BaseException):
                from core.generator import GlobalData
                raise report_exception.FileException(
                    f"文件下载地址已经失效【{GlobalData.system_parameters.__dict__[DownloadServerType.K_DOCS]}】（URL有效期：10分钟，请尝试通过AsyncFetcher重新获取）")
        ...
        if excel_file_path is None:
            raise report_exception.FileException(
                f"文件不存在：{SUMMARY_EXCEL_NAME}，或该文件下载地址已经失效（URL有效期：10分钟，请尝试通过AsyncFetcher重新获取）")
